...
time $GATK VariantRecalibrator \
     -R $REF \
     -V $OUTDIR/gatk/${SAMPLE}.merged.raw.g.vcf.gz \
...
time $GATK ApplyVQSR \
     ...
     --mode SNP \
     -O $OUTDIR/gatk/${SAMPLE}.HC.snps.VQSR.g.vcf.gz && echo "** SNPs VQSR done **"

time $GATK VariantRecalibrator \
     -R $REF \
     -V $OUTDIR/gatk/${SAMPLE}.HC.snps.VQSR.g.vcf.gz \
...
time $GATK ApplyVQSR \
     ...
     --mode INDEL \
     -O $OUTDIR/gatk/${SAMPLE}.HC.VQSR.g.vcf.gz && echo "** SNPs and Indels VQSR (${SAMPLE}) finish done**"

####Extract SNPs
time $GATK SelectVariants \
     -R $REF \
     -V $OUTDIR/gatk/${SAMPLE}.HC.VQSR.g.vcf.gz \
     -select-type SNP \
     -O $OUTDIR/gatk/${SAMPLE}.HC.VQSR.snp.g.vcf.gz
...
time $GATK SelectVariants \
     ...
     -V $OUTDIR/gatk/${SAMPLE}.HC.VQSR.g.vcf.gz \
     -select-type INDEL \
     -O $OUTDIR/gatk/${SAMPLE}.HC.VQSR.indel.g.vcf.gz
