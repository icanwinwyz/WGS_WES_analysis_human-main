...
time $ANNO_PATH/table_annovar.pl \
     $OUTDIR/gatk/${SAMPLE}.HC.VQSR.snp.g.vcf.gz \
     $ANNO_PATH/humandb \
     -buildver hg38 \
...
Rscript $SCRIPT_PATH/WGS_advance_anno.human.format.R $OUTDIR/gatk/${SAMPLE}.HC.VQSR.snp.g.vcf.gz ${SAMPLE}.HC.VQSR.snp $OUTDIR/gatk


####Annotate INDELs
time $ANNO_PATH/table_annovar.pl \
     $OUTDIR/gatk/${SAMPLE}.HC.VQSR.indel.g.vcf.gz \
     $ANNO_PATH/humandb \
     -buildver hg38 \
...
Rscript $SCRIPT_PATH/WGS_advance_anno.human.format.R $OUTDIR/gatk/${SAMPLE}.HC.VQSR.indel.g.vcf.gz ${SAMPLE}.HC.VQSR.indel $OUTDIR/gatk
